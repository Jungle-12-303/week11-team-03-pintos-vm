...
The code in `threads/mmu.c` is an abstract interface to the x86_64 hardware page table. The page
table in Pintos (which is the one you are going to use in the project)
is called `pml4` for Page-Map-Level-4 by Intel processor documentation because the table has
...
x86_64 hardware provides some assistance for implementing page replacement algorithms,
